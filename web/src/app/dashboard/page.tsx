  ...
  return (
    <>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2 sm:gap-3">
      ...
      <div className="grid gap-3 sm:gap-6 grid-cols-2 lg:grid-cols-4 mt-4 sm:mt-6">
        {stats.map((stat, i) => (
          <Card key={i} className="relative overflow-hidden group hover:border-primary/50 transition-all duration-300 premium-shadow border-slate-100/50">
            ...
            <CardContent className="pt-0 pb-4 sm:py-4">
              <div className="text-xl sm:text-3xl font-extrabold text-slate-900 mb-1">{stat.value}</div>
              <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap">
                <span className={`text-[10px] sm:text-xs font-bold px-1.5 sm:px-2 py-0.5 rounded-full ${stat.trendUp ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'} flex items-center`}>
                   {stat.trendUp ? <ArrowUpRight className="h-2.5 w-2.5 sm:h-3 sm:w-3 mr-0.5" /> : null}
                   {stat.trend}
                </span>
                <span className="hidden sm:inline text-[10px] text-slate-400 font-medium">vs last month</span>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="grid gap-3 sm:gap-6 md:grid-cols-2 lg:grid-cols-7 mt-6 sm:mt-8">
        <Card className="col-span-4 glass-panel border-slate-100/50 premium-shadow">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-lg font-bold text-slate-800">Revenue Performance</CardTitle>
              <p className="text-sm text-slate-500 font-medium">Daily revenue trends across your venues</p>
            </div>
        ...
        <Card className="col-span-3 glass-panel border-slate-100/50 premium-shadow overflow-hidden">
      ...
      <div className="grid gap-3 sm:gap-6 sm:grid-cols-2 md:grid-cols-3 mt-6 sm:mt-8">
         <Card className="glass-panel premium-gradient overflow-hidden border-none text-white relative">
         ...
         <Card className="glass-panel border-primary/20 bg-primary/5">
            <CardHeader>
              <CardTitle className="text-lg font-bold text-slate-800">Top selling category</CardTitle>
            </CardHeader>
            <CardContent>
               <div className="flex items-center gap-4">
         ...
         <Card className="glass-panel border-slate-100/50 premium-shadow">
            <CardHeader>
            ...
            <CardContent>
               <div className="space-y-4">
                  {[
                    { name: "Owner", role: "Admin", score: 100, img: "OW" },
